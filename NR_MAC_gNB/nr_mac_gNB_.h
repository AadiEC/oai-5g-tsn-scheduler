...
#define NR_SLOT_DURATION_US(mu) (1000UL << (mu))  // μs per slot for given numerology(Modified)
...
//**********Modifications : CBS (full) + TAS***********************************************
typedef struct {
    uint32_t start_time;  // us
    uint32_t end_time;    // us
    uint8_t traffic_class;
    uint8_t gate_state;   // 1=open, 0=closed
} NR_GCL_Entry_t;

typedef struct {
    int32_t credit;       // Current credit for this traffic class
    int32_t idle_slope;   // Credit increase rate (per us)
    int32_t send_slope;   // Credit decrease rate (per us)
    int32_t hi_limit;     // Upper credit bound
    int32_t lo_limit;     // Lower credit bound
    int dl_buffer_status;
    bool is_high_priority;
    uint32_t burst_counter;
  } NR_CBS_Config_t;

typedef struct {
    uint8_t num_traffic_classes;
    uint32_t gcl_interval;           // Gate Control List interval (us)
    uint64_t next_gcl_update;        // Timestamp for next GCL update (us)
    uint8_t *current_gate_state;     // Current gate states per traffic class
    NR_GCL_Entry_t *gcl;             // Gate Control List entries
    int gcl_length;                  // Number of GCL entries
    NR_CBS_Config_t *cbs_config;     // CBS config per traffic class
} tas_cbs_config_t;
//******Modifications End */
...
typedef struct gNB_MAC_INST_s {
  ...
  //Modifications
  //uint8_t traffic_class; // Traffic class for CBS+TAS (e.g., 0=high, 1=low)
  tas_cbs_config_t tas_cbs; 
  //Modification End
